...
def data_input(data):
    """
    Take in an input file and parses all core temps.

    Args:
        data: an input file
    Yields:
        A list containing a floating point entry of floating point values from the inputted
        file
    """
...
def convert(file):
    """
    Take in an input file and converts said file.
    ...
    with open(file_name, 'r+') as file:
        ...
        file.write(re.sub(r'[+°C]', '', new_text))
        file.truncate()
        file.close()


# based removal of specific characters from
...
def cores():
    ...
    core_1 = open("core-1.txt", "a")
    core_2 = open("core-2.txt", "a")
    core_3 = open("core-3.txt", "a")
    core_4 = open("core-4.txt", "a")
    for row in temps:
        core_1.write(row[0] + "\n")
        core_2.write(row[1] + "\n")
        core_3.write(row[2] + "\n")
...
def interpolation():
    ...
    for c in range(1, 5):
        ...
        inter_file = str(file_notxt + "-core-" + core_num + ".txt")
...
def matrixX(data):
    """
       Take in an list that contains all of the CPU temperatures that were provided from the input file. Each index of
       this list contain four CPU temperatures.

       Args:
           data: list of 4 CPU temperatures values per index in the list.
       Yields:
           Matrix X in the form of a list to be used for least squares calculation.
       """
    xMatrix = []
...
# based pseudocode from https://www.cs.odu.edu/~tkennedy/cs417/latest/Assts/matrixSolverExercise/index.html
